...
def test_a_word_about_a_second_account_never_reaches_a_slot():
    """One Claude account per slot. A server from the short while a slot could
    hold several would still say where a sign-in lands and which account to
    switch to; neither goes any further than this."""
    [slot] = machine.wanted_slots({"slots": [
        {"unix_user": "slot01", "state": "active", "login": {**LOGIN, "account": "new"},
         "account": {"action": "use", "id": "2", "requested_at": 43.0}}]})
    assert slot["login"] == LOGIN
